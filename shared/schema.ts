...
// Customer Account System for Payment Tracking
export const customers = pgTable("customers", {
  ...
  companyName: text("company_name"),
  planId: text("plan_id"), // small-medical, clinics, hospitals
...
export const oneTimePurchases = pgTable("one_time_purchases", {
  ...
  feeType: text("fee_type").notNull(), // 'admin', 'comprehensive_report', 'annual_assessment'
  feeAmount: integer("fee_amount").notNull(), // Amount in cents
  description: text("description").notNull(),
  stripePaymentIntentId: text("stripe_payment_intent_id"),
  status: text("status").default("completed"), // completed, failed, refunded
  purchasedAt: timestamp("purchased_at").defaultNow(),
});

// Subscription Items Tracking
export const subscriptionItems = pgTable("subscription_items", {
  id: serial("id").primaryKey(),
  customerId: integer("customer_id").references(() => customers.id),
  itemType: text("item_type").notNull(), // 'base_plan', 'infrastructure', 'monthly_addons'
  itemName: text("item_name").notNull(),
  monthlyAmount: integer("monthly_amount").notNull(), // Amount in cents
...
// Export types for customer system
