...
export default function EnhancedReportDisplay({ report, onBack }: EnhancedReportDisplayProps) {
  ...
  const handleDownloadReport = () => {
    ...
    try {
      ...
      doc.text("HOS²A Security Assessment Report", 14, 20);
      
      // Report meta information
      doc.setFontSize(12);
      doc.setTextColor(0, 0, 0);
      doc.text(`Company: ${report.businessName || 'N/A'}`, 14, 30);
      doc.text(`Assessment Type: ${report.reportType === 'comprehensive' ? 'Comprehensive' : 'Preliminary'}`, 14, 37);
      doc.text(`Generated: ${formatDate(report.createdAt)}`, 14, 44);
      doc.text(`Assessment ID: ${report.id}`, 14, 51);
      
      // RASBITA Score Section
      doc.setFillColor(245, 245, 245);
      doc.rect(14, 56, 182, 12, 'F');
      doc.setFontSize(12);
      doc.setFont("helvetica", "bold");
      doc.text("RASBITA™ Overall Score", 16, 64);
      doc.setFont("helvetica", "normal");
      
      const overallScore = report.rasbitaScore?.overall || 0;
      let scoreColor;
      ...
      if (overallScore >= 80) {
        ...
        scoreColor = [253, 126, 20]; // Orange
      } else {
        scoreColor = [220, 53, 69]; // Red
      }
      
      doc.setTextColor(...scoreColor);
      doc.text(`${overallScore}%`, 100, 64);
      doc.setTextColor(0, 0, 0);
      ...
      doc.setFontSize(14);
      doc.setFont("helvetica", "bold");
      doc.text("Risk Summary", 14, yPosition);
      ...
      const riskData = [
        ['Risk Level', 'Count', 'Probability'],
        ['Critical', report.summary?.criticalVulnerabilities || 0, '>80%'],
        ['High', report.summary?.highRisks || 0, '60-80%'],
        ['Medium', report.summary?.mediumRisks || 0, '30-60%'],
        ['Low', report.summary?.lowRisks || 0, '<30%']
      ];
      ...
      doc.text('Generated by CyberLockX - Healthcare Apps & Devices Security Hub', 14, pageHeight - 10);
      doc.text('Securing every CLICK!!!', 14, pageHeight - 6);
      
      // Save the PDF
      doc.save(`${report.businessName || 'Assessment'}_HOS2A_Report_${new Date().toISOString().split('T')[0]}.pdf`);
